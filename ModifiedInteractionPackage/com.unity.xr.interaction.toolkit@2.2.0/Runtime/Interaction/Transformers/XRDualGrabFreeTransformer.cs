...
namespace UnityEngine.XR.Interaction.Toolkit.Transformers
{
    ...
    public class XRDualGrabFreeTransformer : XRBaseGrabTransformer
    {
        ...
        // Variable to remember the initial distance between two selecting controllers
        public float initialControllersDistance;
        ...
        public override void OnGrabCountChanged(XRGrabInteractable grabInteractable, Pose targetPose, Vector3 localScale)
        {
            ...
            if (grabInteractable.interactorsSelecting.Count == 2)
            {
                ...
                // Records the positions of the controllers when detecting a dual grab
                Vector3 firstSelectingController = grabInteractable.interactorsSelecting[0].GetAttachTransform(grabInteractable).GetWorldPose().position;
                Vector3 secondSelectingController = grabInteractable.interactorsSelecting[1].GetAttachTransform(grabInteractable).GetWorldPose().position;

                initialControllersDistance = Vector3.Distance(firstSelectingController, secondSelectingController);
                Debug.Log("distance: " + initialControllersDistance);
                initialScale = grabInteractable.gameObject.transform.localScale;

                //test 
                grabInteractable.gameObject.transform.localScale = new Vector3(3, 3, 3);
        ...
        void UpdateTargetMulti(XRGrabInteractable grabInteractable, ref Pose targetPose)
        {
            ...
            float currentControllersDistance = Vector3.Distance(primaryAttachPose.position, secondaryAttachPose.position);
